...
const animateCharPreview = () => {
    ...
    if (charPreviewMesh)
        charPreviewMesh.rotation.y += 0.02;
        ...
        if (itemName) {
            ...
            if (isFood) {
                ...
                if (itemName.includes('apple'))
                    ...
                    icon = '🧀';
                else if (itemName.includes('hot-dog'))
                    icon = '🌭';
                ...
                slotDiv.onclick = () => {
                    ...
                    if (current > 1)
                        playerInventory.set(itemName, current - 1);
                    else
...
(function (BlockType) {
    ...
    BlockType[BlockType["SILVER"] = 8] = "SILVER";
    BlockType[BlockType["DIAMOND"] = 9] = "DIAMOND";
})(BlockType || (BlockType = {}));
const idToName = {
...
const spawnEntities = (cx, cz, chunk) => {
    const key = `${cx},${cz}`;
    if (entities.has(key))
        return;
    const chunkEntities = [];
    const jobs = ['farming', 'mining', 'building', 'pathmaking', 'idle'];
    for (let i = 0; i < 3; i++) {
        if (Math.random() < 0.4) {
        ...
        for (let rz = 0; rz < CHUNK_SIZE; rz++) {
            ...
            for (let y = 0; y <= h && y < CHUNK_HEIGHT; y++) {
                ...
                if (y === h) {
                    ...
                    if (y < 4 && oreNoise < 0.05)
                        type = BlockType.DIAMOND;
                    else if (y < 8 && oreNoise < 0.1)
...
const generateHouse = (x, y, z, style = 1) => {
            ...
            for (let dz = -2; dz <= 2; dz++) {
                for (let dy = -1; dy < 4; dy++) {
                    ...
                    if (by < 0 || by >= CHUNK_HEIGHT)
                        continue;
                    if (dy === -1)
                        setBlock(bx, by, bz, BlockType.WOOD); // Floor
                    else if (dy === 3) { // Roof
                        if (Math.abs(dx) <= 1 && Math.abs(dz) <= 1)
                            setBlock(bx, by, bz, BlockType.WOOD);
                    }
...
const createSetting = (label, min, max, val, onChange) => {
...
gyroBtn.onclick = async () => {
    if (!useGyro) {
        if (typeof DeviceOrientationEvent.requestPermission === 'function') {
            ...
            if (res !== 'granted')
                return alert('Gyro permission denied');
        }
        useGyro = true;
...
gyroRow.appendChild(gyroLabel);
gyroRow.appendChild(gyroBtn);
settingsScreen.appendChild(gyroRow);
window.addEventListener('deviceorientation', (e) => {
    if (!useGyro || !isGameStarted || isSettingsOpen)
        return;
...
const createHouseBtn = (label, style) => {
    ...
    if (isGameStarted && !isSettingsOpen && !isInventoryOpen) {
        if (e.button === 0) { // Left click
            ...
            if (intersects.length > 0) {
                ...
                while (obj.parent && !entMap.has(obj))
                    obj = obj.parent;
...
const updateHunger = (delta) => {
    ...
    if (hunger < 30)
        hungerBar.style.background = '#f44336';
    else if (hunger < 60)
        hungerBar.style.background = '#ff9800';
    else
        hungerBar.style.background = '#4CAF50';
};
...
function animate() {
    ...
    if (ctrl.isLocked || isTouchDevice) {
        ...
        if (isFlying) {
            ...
            let onGround = false;
            const checkPoints = [[px - 0.2, nextFootY, pz - 0.2], [px + 0.2, nextFootY, pz - 0.2], [px - 0.2, nextFootY, pz + 0.2], [px + 0.2, nextFootY, pz + 0.2]];
                ...
                if (ent.type === 'character') {
                    ...
                    if (ent.onGround) {
                        if (!ent.targetPos || (ent.moveTimer && now > ent.moveTimer)) {
                            ...
                            if (ent.job === 'mining')
                                dist = 1; // Stay near current spot
                            else if (ent.job === 'idle')
                                dist = 0.5;
                            ent.targetPos = new THREE.Vector3(ent.mesh.position.x + Math.cos(angle) * dist, ent.mesh.position.y, ent.mesh.position.z + Math.sin(angle) * dist);
